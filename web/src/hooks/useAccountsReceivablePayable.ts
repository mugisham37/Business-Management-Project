...
export function useAccountsReceivable(filters: ReceivableFilters = {}) {
  ...
  const receivablesSummary = useMemo(() => {
    ...
    const totalOutstanding = receivables.reduce((sum, r) => sum + r.balanceAmount, 0);
    const overdueReceivables = receivables.filter(r => r.isOverdue);
    const totalOverdue = overdueReceivables.reduce((sum, r) => sum + r.balanceAmount, 0);
    
    const byStatus = receivables.reduce((acc, r) => {
      acc[r.status] = (acc[r.status] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const byAging = receivables.reduce((acc, r) => {
      acc[r.agingCategory] = (acc[r.agingCategory] || 0) + r.balanceAmount;
      return acc;
    }, {} as Record<string, number>);
    ...
    return {
      ...
      averageDaysOverdue: overdueReceivables.length > 0 ? 
        overdueReceivables.reduce((sum, r) => sum + r.daysOverdue, 0) / overdueReceivables.length : 0,
      byStatus,
      byAging,
    };
  }, [receivables]);
...
export function useAccountsPayable(filters: PayableFilters = {}) {
  ...
  const payables = useMemo(() => {
    ...
    return data.getPayables.map((payable: any) => {
      const totalAmount = parseFloat(payable.totalAmount || '0');
      const paidAmount = parseFloat(payable.paidAmount || '0');
      const balanceAmount = parseFloat(payable.balanceAmount || '0');
  ...
  const payablesSummary = useMemo(() => {
    ...
    const totalOutstanding = payables.reduce((sum, p) => sum + p.balanceAmount, 0);
    const overduePayables = payables.filter(p => p.isOverdue);
    const totalOverdue = overduePayables.reduce((sum, p) => sum + p.balanceAmount, 0);
    const dueSoonPayables = payables.filter(p => p.isDueSoon);
    const totalDueSoon = dueSoonPayables.reduce((sum, p) => sum + p.balanceAmount, 0);
    
    const byStatus = payables.reduce((acc, p) => {
      acc[p.status] = (acc[p.status] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const byAging = payables.reduce((acc, p) => {
      acc[p.agingCategory] = (acc[p.agingCategory] || 0) + p.balanceAmount;
      return acc;
    }, {} as Record<string, number>);
    ...
    return {
      ...
      averageDaysOverdue: overduePayables.length > 0 ? 
        overduePayables.reduce((sum, p) => sum + p.daysOverdue, 0) / overduePayables.length : 0,
      byStatus,
      byAging,
    };
  }, [payables]);
...
export function useAgingReport(reportType: 'receivables' | 'payables', asOfDate?: Date) {
  ...
  const agingReport = useMemo(() => {
    ...
    return data.getAgingReport.map((item: any) => ({
      ...item,
      totalAmount: parseFloat(item.totalAmount || '0'),
      current: parseFloat(item.current || '0'),
      days1to30: parseFloat(item.days1to30 || '0'),
  ...
  const agingSummary = useMemo(() => {
    ...
    return {
      ...
      highRiskEntities: agingReport.filter(item => item.riskLevel === 'high').length,
      mediumRiskEntities: agingReport.filter(item => item.riskLevel === 'medium').length,
      lowRiskEntities: agingReport.filter(item => item.riskLevel === 'low').length,
    };
  }, [agingReport]);
...
export function useARAPMutations() {
  ...
  const [recordReceivablePaymentMutation] = useMutation(RECORD_RECEIVABLE_PAYMENT, {
    onError: (error) => {
      errorLogger.logError(error, {
        component: 'useARAPMutations',
        operation: 'recordReceivablePayment',
        tenantId: currentTenant?.id,
      });
    },
  });
  ...
  const [recordPayablePaymentMutation] = useMutation(RECORD_PAYABLE_PAYMENT, {
    onError: (error) => {
      errorLogger.logError(error, {
        component: 'useARAPMutations',
        operation: 'recordPayablePayment',
        tenantId: currentTenant?.id,
      });
    },
  });
...
export function useARAPSubscriptions() {
  const { currentTenant } = useAuth();
  const [arapNotifications, setArapNotifications] = useState<any[]>([]);
...
function calculateRiskLevel(item: any): 'low' | 'medium' | 'high' {
  const totalAmount = parseFloat(item.totalAmount || '0');
  const over90Days = parseFloat(item.over90Days || '0');
  const days61to90 = parseFloat(item.days61to90 || '0');
