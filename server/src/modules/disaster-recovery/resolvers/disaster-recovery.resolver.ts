...
import { UseGuards, Logger, UseInterceptors, Inject } from '@nestjs/common';
...
export class DisasterRecoveryResolver {
  ...
  async executeFailover(
    ...
    try {
      ...
      const executionData: any = {
        ...
        status: execution.success ? FailoverStatus.COMPLETED : FailoverStatus.FAILED,
        isAutomatic: false,
        startedAt: new Date(),
      ...
      if (input.reason !== undefined) executionData.reason = input.reason;
      if (execution.success) executionData.completedAt = new Date();
      if (!execution.success) executionData.error = execution.errors.join(', ');
  ...
  async generateDRReport(
    ...
    try {
      ...
      if (input.startDate !== undefined) reportOptions.startDate = input.startDate;
      if (input.endDate !== undefined) reportOptions.endDate = input.endDate;
      
      const report = await this.dataManagementService.generateDRReport(reportOptions);
  ...
  drExecutionStarted(@Context() context: any) {
    return (this.pubSub as any).asyncIterator('drExecutionStarted');
  }
  ...
  drExecutionCompleted(@Context() context: any) {
    return (this.pubSub as any).asyncIterator('drExecutionCompleted');
  }
  ...
  failoverExecuted(@Context() context: any) {
    return (this.pubSub as any).asyncIterator('failoverExecuted');
  }
  ...
  replicationLagWarning(@Context() context: any) {
    return (this.pubSub as any).asyncIterator('replicationLagWarning');
  }
  ...
  drRecoveryCompleted(@Context() context: any) {
    return (this.pubSub as any).asyncIterator('drRecoveryCompleted');
  }
  ...
  @ResolveField(() => [String])
  async planReplicationStatus(@Parent() plan: DisasterRecoveryPlan): Promise<string[]> {
    try {
  ...
  private async publishDREvent(eventType: string, data: any): Promise<void> {
    try {
      (this.pubSub as any).publish(eventType, { [eventType]: data });
    } catch (error) {
