  ...
  private createMockQueueItems(tenantId: string, deviceId?: string, count: number = 5): OfflineTransactionQueue[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      queueItems.push({
        ...
        transactionData: {
          operation: {
            type: ['create_transaction', 'update_transaction', 'void_transaction'][Math.floor(Math.random() * 3)] || 'create_transaction',
            data: { amount: Math.round(Math.random() * 100 * 100) / 100 },
            timestamp: new Date(Date.now() - i * 60000),
          },
        },
        operationType: (['create_transaction', 'update_transaction', 'void_transaction'][Math.floor(Math.random() * 3)] || 'create_transaction') as string,
        isSynced,
        syncAttempts: isSynced ? 1 : Math.floor(Math.random() * 3),
        ...(isSynced && { lastSyncAttempt: new Date(Date.now() - i * 30000) }),
        ...(isSynced && { syncedAt: new Date(Date.now() - i * 30000) }),
        syncErrors: [],
